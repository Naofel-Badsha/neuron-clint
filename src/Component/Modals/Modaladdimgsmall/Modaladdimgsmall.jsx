...
export const Modaladdimgsmall = ({ openModal, setOpenModal, refetch }) => {
    const AxiosCommon = useAxiosCommon();
    const [showName, setShowName] = useState({});
    const [showImagePreview, setShowImagePreview] = useState('');
    const fileInputRef = useRef();

    const cloudinary = new Cloudinary({ cloud_name: 'dbheixvje', secure: true });
    ...
    const onSubmit = async (event) => {
        ...
        if (showName.name) {
            ...
            formData.append('upload_preset', 'neuronnarsing'); // Replace with your Cloudinary upload preset

            try {
                const cloudinaryResponse = await axios.post(
                    `https://api.cloudinary.com/v1_1/dbheixvje/image/upload`, 
                    formData
                );
                ...
                try {
                    const response = await AxiosCommon.post('/addslidersimgsmoll', { image: imageUrl });
