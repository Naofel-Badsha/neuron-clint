...
const ManageSuccessfulstudents = () => {
  ...
  const handleDelete = () => {
    AxiosCommon.delete(`/Successfulstudents/${itemToDelete}`)
      .then((res) => {
        refetch();
        toast.success("corse deleted successfully!", {
          position: "top-center",
        });
        closeModal();
      })
      .catch((err) => {
        console.error(err);
        toast.error("Failed to delete corse", { position: "top-center" });
      });
  };
      ...
      {corses.map((data, idx) => (
        <div key={idx} className="flex gap-7 mt-6 border-b-2 pb-3 items-center">
          ...
          <div className="">
            ...
            <div className=" text-base mt-3 font-semibold text-center max-w-48">
              {data.name}
            </div>
          </div>
...
export default ManageSuccessfulstudents;
