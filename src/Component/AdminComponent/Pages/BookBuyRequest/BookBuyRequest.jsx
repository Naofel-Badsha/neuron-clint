...
const BookBuyRequest = () => {
  ...
  const { data: users = [], isLoading, refetch } = useQuery({
    queryKey: ["Bookbuy"],
    queryFn: async () => {
      const { data } = await AxiosCommon(`/Bookbuy/pending`); // Fetch users with pending status
      return data;
    },
  });
  ...
  // Handle admission change to "admit"
  const handleAdmit = (userId) => {
    AxiosCommon.patch(`/Bookbuy/${userId}`, { order: "complate" })
      .then((res) => {
        refetch(); // Refetch to update UI after admitting
        toast.success('User admitted successfully!', { position: 'top-center' });
      })
      .catch((err) => {
        console.error(err);
        toast.error('Failed to admit user', { position: 'top-center' });
      });
  };
        ...
        <table className="min-w-full bg-white border border-gray-300">
          <thead>
            <tr>
              ...
              <th className="px-4 py-2 border">bookName</th>
              ...
              <tr key={user._id}>
                ...
                <td className="px-4 py-2 border text-center">
                  ...
                  <button
                    ...
                    onClick={() => handleAdmit(user._id)} // Admit user
                  >
                    Admit
                  </button>
                </td>
